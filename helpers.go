...
// Ensures that the pivot element in the given matrix m at the specified row is non-zero.
...
// Divides each element of the given row slice by the provided pivot value.
...
// Subtracts row-2 (r2) from row-1 (r1) scaled by the provided factor value.
